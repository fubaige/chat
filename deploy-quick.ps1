...
$remoteCmd = "cd $DEPLOY_PATH && tar -xzf deploy-archive.tar.gz 2>/dev/null && sed -i 's/\r$//' *.sh 2>/dev/null && chmod +x *.sh 2>/dev/null && rm -f deploy-archive.tar.gz && export DOCKER_BUILDKIT=1 && export COMPOSE_DOCKER_CLI_BUILD=1 && docker-compose up -d --build"
