...
if grep -q "dbms.connector.bolt.listen_address" "$CONF_FILE"; then
  ...
  echo "dbms.connector.bolt.listen_address=:7687" >> "$CONF_FILE"
fi

echo "Config updated to allow remote connections (0.0.0.0:7687)"
...
if command -v ufw >/dev/null; then
  ...
  echo "Warning: No known firewall tool found (ufw/firewalld/iptables). Please open ports 7687/7474 manually."
fi
...
echo "   You can now connect to Neo4j via: bolt://<YOUR_SERVER_IP>:7687"
echo "==========================================="
