...
function addMessageToUI(type, content, imageUrl, isLoading = false) {
    ...
    const parsedContent = isLoading ? content : marked.parse(content);
...
async function loadKbFiles(silent = false) {
    ...
    try {
        ...
        if (!silent) {
            elements.kbFileList.innerHTML = '<tr><td colspan="6" style="text-align:center; color:#cf6679;">加载失败</td></tr>';
        }
    }
}
...
function renderKbTable(files) {
    ...
    elements.kbFileList.innerHTML = files.map(file => {
        const isReady = file.status === 'success';
        const nameHtml = isReady
            ? `<a href="javascript:void(0)" onclick="viewKbChunks(${file.id}, '${file.original_name.replace(/'/g, "\\'")}')" style="color:#4d6bfe; text-decoration:none; cursor:pointer;" title="点击查看文本块内容"><i class="far fa-file-alt" style="margin-right:8px;"></i>${file.original_name}</a>`
        ...
        <tr>
            ...
            <td>
                ${isReady ? `<button onclick="viewKbChunks(${file.id}, '${file.original_name.replace(/'/g, "\\'")}')" style="background:none; border:none; color:#4d6bfe; cursor:pointer; margin-right:8px;" title="查看内容"><i class="fas fa-eye"></i></button>` : ''}
