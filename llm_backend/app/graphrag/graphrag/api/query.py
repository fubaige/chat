...
# 强制使用项目内版本的 get_embedding_store（系统包接口已变更，不兼容）
...
def local_search_streaming(
    ...
    # 调试：确认向量表是否成功打开
    _dc = getattr(description_embedding_store, 'document_collection', None)
    _cn = getattr(description_embedding_store, 'collection_name', '?')
    logger.info(f"[DEBUG] entity_description store: collection_name={_cn}, document_collection={'OK' if _dc is not None else 'NONE/未找到表'}")
