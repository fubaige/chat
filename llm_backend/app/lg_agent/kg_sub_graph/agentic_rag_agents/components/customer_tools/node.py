...
def create_graphrag_query_node():
    ...
    async def graphrag_query(state, config: RunnableConfig = None):
        ...
        logger.info(f"GraphRAG query for user_id: {user_id}")
        
        final_response = ""
        
        # Step 1: 查询用户自己的知识库
        user_storage_dir = _get_user_storage_dir(user_id)
        logger.info(f"Step 1: Trying user storage dir: {user_storage_dir}, exists: {os.path.exists(user_storage_dir)}")
        
        final_response = await _try_graphrag_query(task, user_storage_dir)
        ...
        if final_response and final_response.strip():
            ...
            # Step 2: 非 admin 用户，尝试 admin 公共知识库
