from typing import List, Dict, AsyncGenerator, Callable, Optional, Union
...
class DeepseekService:
    def __init__(self, model: str = "deepseek-chat", api_key: str = None, base_url: str = None, model_name: str = None):
        logger.info("Initializing Deepseek Service")
        self.client = AsyncOpenAI(
            api_key=api_key or settings.DEEPSEEK_API_KEY,
            base_url=base_url or settings.DEEPSEEK_BASE_URL
        )
        # 优先使用传入的 model_name，其次配置中的 DEEPSEEK_MODEL，最后默认值
        self.model = model_name or settings.DEEPSEEK_MODEL or model
    ...
    async def ainvoke(self, input_data: Union[str, List[Dict]]) -> object:
        """
        兼容 LangChain 风格的异步调用接口。
        
        Args:
            input_data: 可以是字符串提示词，也可以是符合 OpenAI 规范的消息列表。
            
        Returns:
            返回一个带有 content 属性的对象，模拟 LangChain 的 AIMessage。
        """
        logger.info(f"DeepseekService.ainvoke 开始执行，输入类型: {type(input_data)}")
        ...
        try:
            ...
            logger.info(f"DeepseekService.ainvoke 生成成功，内容长度: {len(content) if content else 0}")
            ...
            logger.error(f"DeepseekService.ainvoke 异常: {str(e)}")
            raise
