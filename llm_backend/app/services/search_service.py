...
class SearchService:
    ...
    async def search(self, query: str) -> List[Dict]:
        ...
        """通用的搜索接口，供 WechatService 等调用"""
        logger.info(f"SearchService.search 被调用，查询词: {query}")
        try:
            results = await self._handle_search(query)
            logger.info(f"SearchService.search 执行完成，获取到 {len(results)} 条结果")
            return results
        except Exception as e:
            logger.error(f"SearchService.search 异常: {str(e)}")
            return []