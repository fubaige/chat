...
5. **知识库索引优化** (2026-03-07):
   - 将 GraphRAG LLM (API Base/Key/Model) 和 Embedding 配置接入数据库动态配置系统。
   - 解决了 GraphRAG 索引时因环境变量缺失导致的 `KeyError: 'GRAPHRAG_API_BASE'` 问题。
   - 移除了对本地 `graphrag/data/.env` 的依赖，实现全数据库驱动配置。
   - 增加了索引构建时的详细调试日志输出。
6. **极简运维与持续集成**
