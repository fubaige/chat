...
class WxRspMsg(object):
    ...
    def insert_elem(self, name, value, is_cdata=True):
        ...
        if is_cdata and value:
            curr_node.text = f"<![CDATA[{value}]]>"
        else:
    ...
    def update_xml(self):
        ...
        self.insert_elem("CreateTime", self.create_time, is_cdata=False)
        self.insert_elem("MsgType", self.msg_type)

    def dump_xml(self):
        self.update_xml()
        import html
        # 使用 standard xml 避免 html 转义问题，随后手动 unescape CDATA 标记
        xml_str = ET.tostring(self.xml_tree, encoding="utf-8", method="xml").decode("utf-8")
