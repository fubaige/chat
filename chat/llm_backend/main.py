from fastapi import FastAPI, HTTPException, UploadFile, File, Query, Form, Depends, Request
...
from app.lg_agent.lg_builder import _sanitize_emoji
import json
...
# 挂载静态文件服务，使 /uploads/... 可通过 HTTP 公网访问（MinerU 需要能访问文件 URL）
# 注意：必须在 app 创建后、路由注册前挂载
...
# 挂载 uploads 静态文件目录，使 /uploads/... 可通过 HTTP 公网访问
# MinerU 需要能通过公网 URL 访问上传的文件
...
# 2. 内部 API 路由直接挂载（/internal/ 前缀已在 router 中定义）
...
async def startup_event():
    ...
    try:
        ...
        logger.warning(f"Startup: 可选模块未安装，跳过初始化: {e}")
    except Exception as e:
    ...
    try:
        async with AsyncSessionLocal() as db:
            ...
            result = await db.execute(
                select(WechatMpConfig).where(WechatMpConfig.id == config_id)
            )
...
async def _handle_wechat_async(body: bytes, signature: str, timestamp: str, nonce: str, config_id: int):
    ...
    try:
        async with AsyncSessionLocal() as db:
            result = await db.execute(select(WechatMpConfig).where(WechatMpConfig.id == config_id))
            config = result.scalar_one_or_none()
            if not config: return
            
            service = WechatService(config)
            xml_body = body.decode("utf-8")
            msg = service._parse_xml_message(xml_body)
            openid = msg.get("FromUserName")
            user_content = msg.get("Content", "")
            
            if openid and user_content:
                ai_reply = await service._get_ai_reply(user_content, db)
                await service.send_custom_message(openid, ai_reply)
    except Exception as e:
        logger.error(f"微信后台异步处理失败: {e}")
    ...
    try:
        async with AsyncSessionLocal() as db:
            ...
            try:
                ...
                return Response(content=response_content, media_type="text/xml")
            except asyncio.TimeoutError:
                logger.warning(f"微信消息处理超时 (4s)，转入后台异步处理: config_id={config_id}")
...
async def get_knowledge_base(user_id: int, current_user: User = Depends(get_current_user)):
    ...
    try:
        async with AsyncSessionLocal() as session:
            stmt = select(KnowledgeBaseDocument).where(KnowledgeBaseDocument.user_id == user_id).order_by(KnowledgeBaseDocument.created_at.desc())
    ...
    try:
        async with AsyncSessionLocal() as session:
            ...
            kb_result = await session.execute(
                select(KnowledgeBaseDocument).where(KnowledgeBaseDocument.user_id == user_id)
            )
