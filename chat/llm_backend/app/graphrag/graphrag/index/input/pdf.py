...
from graphrag.index.input.util import load_files, generate_image_descriptions_sync
    ...
    async def load_file(path: str, group: dict | None) -> pd.DataFrame:
        ...
        try:
            ...
            try:
                ...
                # 为图片生成描述
                if image_info and image_info.get("images") and config.image_description_api_key:
...
def generate_descriptions_for_images(doc_local_dir, image_info, config):
    """为图片生成描述（使用 Gemini 后端）"""
    if not image_info or not image_info.get("images"):
        return image_info
    
    try:
        # 获取图片所在目录
        # 假设图片路径相对于 doc_local_dir
        image_dir = doc_local_dir
        
        # 如果 auto 目录存在，则在 auto 目录下寻找
        auto_dir = doc_local_dir / "auto"
        if auto_dir.exists():
            image_dir = auto_dir
        ...
        # 调用图片描述生成函数 (已经在 util.py 中实现并通过 config 注入)
        try:
            from graphrag.index.input.util import generate_image_descriptions_sync
            ...
            for image_data in image_info["images"]:
                if image_data.get("path"):
                    img_path = str(image_dir / image_data["path"])
                    if img_path in descriptions:
                        image_data["description"] = descriptions[img_path]
                    else:
                        # 尝试相对路径或文件名匹配
                        ...
                        for k, v in descriptions.items():
                            if k.endswith(rel_path) or Path(k).name == img_name:
                                image_data["description"] = v
                                found = True
                                break
                        
                        if not found:
                            image_data["description"] = "未生成描述"
                else:
                    image_data["description"] = "路径缺失"
        
        except Exception as e:
            log.error(f"Generate image descriptions failed: {e}")
            for image_data in image_info["images"]:
                image_data["description"] = f"Error: {e}"
        
        return image_info
    except Exception as e:
        log.error(f"Error in generate_descriptions_for_images: {e}")
        return image_info