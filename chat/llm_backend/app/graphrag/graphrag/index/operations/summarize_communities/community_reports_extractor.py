...
class FindingModel(BaseModel):
    ...
    summary: str = Field(default="", description="The summary of the finding.")
    explanation: str = Field(default="", description="An explanation of the finding.")

    @classmethod
    def model_validate(cls, obj, *args, **kwargs):
        """容错处理：修复 LLM 偶发的字段名截断问题（如 'mary' 应为 'summary'）"""
        if isinstance(obj, dict):
            # 修复常见截断：'mary' 是 'summary' 被截断的残留
            if "mary" in obj and "summary" not in obj:
                obj = {**obj, "summary": obj.pop("mary")}
            # 确保必要字段存在
