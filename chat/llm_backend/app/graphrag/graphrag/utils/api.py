...
# 使用项目内版本，确保 LanceDBVectorStore 的 connect 逻辑正确
from app.graphrag.graphrag.vector_stores.factory import VectorStoreFactory


# 本地定义，避免依赖系统包中可能不存在的 create_collection_name
# 系统包里对应函数名为 create_index_name，逻辑相同：将 "." 替换为 "-"
# 实际索引时生成的表名示例：default-community-full_content（下划线保留，点变连字符）
def create_collection_name(container_name: str, embedding_name: str, validate: bool = True) -> str:
    """生成向量存储的集合名称（与索引时保持一致）。"""
