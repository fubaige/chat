...
class cypher_query(BaseModel):
    """仅当用户明确要求查询具体的结构化数据（如特定订单号、具体产品库存数量、精确价格等）时才使用此工具。一般性问题请勿使用。"""
...
class microsoft_graphrag_query(BaseModel):
    """默认首选工具。用于查询知识库中的所有内容，包括但不限于：业务方案、营销策略、产品介绍、行业知识、公司信息、技术文档、售后服务、常见问题等。除非问题明确需要查询结构化数据库（如具体订单号、库存数量），否则一律使用此工具。"""
    query: str = Field(..., description="query the graph must include the question")
