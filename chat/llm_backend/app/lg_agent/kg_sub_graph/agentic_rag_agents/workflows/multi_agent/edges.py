...
def map_reduce_planner_to_customer_tools(state: OverallState) -> List[Send]:
    """直接将 planner 的每个子任务映射到 customer_tools（GraphRAG）节点。
    
    绕过 tool_selection 节点，避免双层 Send 嵌套导致 LangGraph FuturesDict 回调为 None 的问题。
    适用于已确定优先使用 GraphRAG 知识库的场景。
    """
    return [
        Send(
            "customer_tools",
            {
                ...
                "query_parameters": {"query": task.question},
                "steps": ["planner"],
            },
        )
        for task in state.get("tasks", list())
