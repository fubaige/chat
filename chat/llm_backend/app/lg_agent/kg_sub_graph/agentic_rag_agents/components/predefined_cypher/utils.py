...
class VectorQueryMatcher:
    ...
    def __init__(
        ...
        # TODO: 集成 DeepSeek 嵌入或其他替代方案
        self.embedding_type = settings.EMBEDDING_TYPE
        print(f"当前嵌入类型: {self.embedding_type}")
        
        # 预计算查询向量
        self.query_vectors = self._compute_query_vectors()
    
    def _embed_texts(self, texts: List[str]) -> List[List[float]]:
        """将文本转换为向量（待集成 DeepSeek 嵌入）"""
        print(f"Warning: Embedding call for {len(texts)} texts. Currently not fully implemented without Ollama.")
        # 如果调用失败，返回空向量作为后备
