...
class IndexingService:
    ...
    async def _extract_text_from_pdf(self, file_path: str) -> str:
        """从 PDF 提取文本（PyPDF2，MinerU 已在上层优先处理）"""
        def _sync_extract():
            try:
                import PyPDF2
                text = ""
    ...
    async def _prepare_input_file(self, file_path: str, file_type: str, input_dir: str) -> str:
        ...
        # MinerU 支持的格式（MIME 类型 + 扩展名双重判断，兼容 octet-stream）
        MINERU_SUPPORTED_MIME = {
            'application/pdf',
            ...
            'image/png',
            'image/webp',
            'application/octet-stream',  # 浏览器有时对 docx/pptx 返回此类型
        }
        MINERU_SUPPORTED_EXT = {'.pdf', '.doc', '.docx', '.ppt', '.pptx', '.jpg', '.jpeg', '.png', '.webp'}
        file_ext = os.path.splitext(file_path)[1].lower()
        is_mineru_supported = file_type in MINERU_SUPPORTED_MIME or file_ext in MINERU_SUPPORTED_EXT

        mineru_token = settings.MINERU_API_TOKEN
        if mineru_token and mineru_token.strip() and is_mineru_supported:
            # 优先用数据库中持久化的服务器地址，其次用本次请求动态检测的地址
            server_base = (settings.SERVER_BASE_URL or self._current_server_base_url or "").rstrip("/").strip()
            if server_base:
                # 只取 uploads/ 之后的相对路径，构造公网可访问的 URL
                # file_path 示例：/www/wwwroot/.../llm_backend/uploads/uuid/ts/file.docx
                # 目标 URL：https://wecom.fubaige.cn/uploads/uuid/ts/file.docx
                from urllib.parse import quote
                norm_path = file_path.replace("\\", "/")
                uploads_marker = "/uploads/"
                idx = norm_path.find(uploads_marker)
                if idx != -1:
                    rel_path = norm_path[idx + 1:]  # uploads/uuid/ts/file.docx
                else:
        ...
        # DOCX → python-docx（按 MIME 或扩展名判断）
        ...
        # PDF → PyPDF2 + Gemini 兜底（按 MIME 或扩展名判断）
    ...
    async def _update_db_record(self, record_id: int, update_data: Dict[str, Any]):
        """更新数据库中的知识库记录"""
        async with AsyncSessionLocal() as session:
            stmt = update(KnowledgeBaseDocument).where(KnowledgeBaseDocument.id == record_id).values(**update_data)
    ...
    async def process_file(self, file_info: Dict[str, Any], record_id: Optional[int] = None) -> Dict[str, Any]:
        ...
        try:
            ...
            # 准备配置路径：优先在 project_dir 下找，其次在 data_dir 下找
            config_path = (self.project_dir / config_file).resolve()
            if not config_path.exists():
                config_path = (self.project_dir / self.default_config).resolve()
            if not config_path.exists():
    ...
    @staticmethod
    def _inject_graphrag_env():
        """将 GraphRAG 所需的环境变量从 settings 注入到当前进程环境。
        
        settings.yaml 中使用 ${GRAPHRAG_API_BASE} 等占位符，
        GraphRAG load_config 会从 os.environ 读取这些变量。
        """
        from app.core.config import settings as _s
        # 确保 API base 以 /v1 结尾（DeepSeek OpenAI 兼容接口）
        deepseek_base = getattr(_s, "DEEPSEEK_BASE_URL", "https://api.deepseek.com").rstrip("/")
        if not deepseek_base.endswith("/v1"):
            deepseek_base = deepseek_base + "/v1"
        env_map = {
            # LLM（DeepSeek OpenAI 兼容接口）
            "GRAPHRAG_API_BASE": deepseek_base,
            "GRAPHRAG_API_KEY": getattr(_s, "DEEPSEEK_API_KEY", ""),
            "GRAPHRAG_MODEL_NAME": getattr(_s, "DEEPSEEK_MODEL", "deepseek-chat"),
            # Embedding（阿里百炼）
            "Embedding_API_BASE": "https://dashscope.aliyuncs.com/compatible-mode/v1",
            "Embedding_API_KEY": getattr(_s, "DASHSCOPE_API_KEY", ""),
            "Embedding_MODEL_NAME": getattr(_s, "EMBEDDING_MODEL", "text-embedding-v4"),
        }
        for k, v in env_map.items():
            if v:  # 只注入非空值
                os.environ[k] = str(v)
        logger.info(f"[线程] 已注入 GraphRAG 环境变量: API_BASE={env_map['GRAPHRAG_API_BASE']}, MODEL={env_map['GRAPHRAG_MODEL_NAME']}, EMBEDDING_MODEL={env_map['Embedding_MODEL_NAME']}")
    ...
    def _run_indexing_sync(
        ...
        # 注入 GraphRAG 所需的环境变量（settings.yaml 中的 ${...} 占位符）
        IndexingService._inject_graphrag_env()
