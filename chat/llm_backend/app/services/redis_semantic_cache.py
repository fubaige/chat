...
class RedisSemanticCache:
    ...
    async def _get_embedding(self, text: str) -> List[float]:
        """获取文本向量，支持 sentence_transformer 和 dashscope 两种模式"""
        try:
            if settings.EMBEDDING_TYPE == "dashscope":
                from openai import AsyncOpenAI
                api_key = settings.DASHSCOPE_API_KEY
                if not api_key:
                    logger.error("DASHSCOPE_API_KEY 未配置，无法使用阿里百炼 Embedding")
                    return [0.0] * 1024
                client = AsyncOpenAI(
                    api_key=api_key,
                ...
                # sentence_transformer 本地模式（需要安装 sentence_transformers）
                try:
                    from sentence_transformers import SentenceTransformer
                except ImportError:
                    raise RuntimeError(
                        "sentence_transformers 未安装。请设置 EMBEDDING_TYPE=dashscope 使用阿里百炼 Embedding。"
                    )
