...
# faiss 为可选依赖（系统默认使用阿里百炼 Embedding，不需要本地 FAISS）
try:
    import faiss
...
class EmbeddingService:
    ...
    def _get_local_model(self):
        """懒加载本地 SentenceTransformer 模型（仅当 EMBEDDING_TYPE != dashscope 时使用）"""
        if self._local_model is None:
            try:
                from sentence_transformers import SentenceTransformer
            except ImportError:
                raise RuntimeError(
                    "sentence_transformers 未安装。系统默认使用阿里百炼 Embedding，"
                    "请确认 EMBEDDING_TYPE=dashscope 且 DASHSCOPE_API_KEY 已配置。"
                )
    ...
    def _create_index(self):
        ...
        if not _FAISS_AVAILABLE:
            raise RuntimeError("faiss-cpu 未安装，无法使用本地向量索引。请使用阿里百炼 Embedding（EMBEDDING_TYPE=dashscope）。")
    ...
    def _save_index(self, file_id: str, index, documents: dict):
        """保存索引和文档数据"""
        try:
            # 使用安全的文件名
