...
check_and_configure_neo4j() {
    ...
    if [ -f "$NEO4J_CONF" ]; then
        ...
        if grep -q "^#server.bolt.listen_address" "$NEO4J_CONF"; then
            sed -i 's/^#server.bolt.listen_address.*/server.bolt.listen_address=0.0.0.0:7688/' "$NEO4J_CONF"
        elif ! grep -q "^server.bolt.listen_address" "$NEO4J_CONF"; then
            echo "server.bolt.listen_address=0.0.0.0:7688" >> "$NEO4J_CONF"
        fi
        echo "      Neo4j 配置已更新"
    fi
...
echo ""
echo "Neo4j 管理："
echo "  - 浏览器界面: http://localhost:7475"
